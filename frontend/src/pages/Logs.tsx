    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700 relative">
            <div className="flex justify-between items-center bg-gradient-to-br from-[#123F2E] to-[#1E5A44] p-8 xl:p-10 rounded-[2rem] text-white shadow-2xl relative overflow-hidden group border border-[#1E5A44]">
                <div className="absolute top-0 right-0 w-72 h-72 bg-emerald-500/30 rounded-full blur-[70px] -translate-y-1/2 translate-x-1/2 group-hover:bg-teal-500/30 transition-colors duration-1000"></div>
                <div className="absolute bottom-0 left-0 w-72 h-72 bg-emerald-500/20 rounded-full blur-[70px] translate-y-1/2 -translate-x-1/4 group-hover:bg-emerald-500/30 transition-colors duration-1000"></div>
                ...
                <div className="relative z-10">
                    <h1 className="text-3xl font-extrabold flex items-center gap-4 tracking-tight">
                        <div className="p-3 bg-white/10 rounded-2xl border border-white/10 shadow-sm backdrop-blur-sm">
                            <FileClock className="w-8 h-8 text-emerald-400 drop-shadow-sm" />
                        </div>
                ...
                <div className="p-8 border-b border-white/50 flex flex-wrap gap-4 justify-between items-center bg-white/40">
                    <div className="relative w-full max-w-md group">
                        ...
                        <input
                            ...
                            className="w-full pl-12 pr-4 py-3.5 bg-white/80 border border-white shadow-[0_2px_15px_rgba(0,0,0,0.02)] rounded-2xl text-sm font-medium focus:outline-none focus:ring-4 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all outline-none"
                        />
                    </div>
                        ...
                        <tbody className="divide-y divide-slate-100/50 text-slate-700">
                            {filteredLogs.map((log, idx) => (
                                <tr key={log.id} className="hover:bg-white/80 transition-all duration-300 group even:bg-white/30 odd:bg-transparent rounded-xl animate-in fade-in slide-in-from-bottom-2" style={{ animationDelay: `${idx * 50}ms`, animationFillMode: 'both' }}>
                                    <td className="p-5 text-sm font-bold text-slate-500 group-hover:text-teal-600 transition-colors first:rounded-l-2xl">
                                    ...
                                    {activeTab === 'out' && (
                                        <td className="p-5">
                                            <span className="px-3 py-1.5 bg-teal-50 border border-teal-100 text-teal-700 font-extrabold text-xs rounded-xl flex max-w-max items-center gap-2 shadow-sm">
                                    ...
                                    <td className="p-5 text-right font-black font-mono text-xl text-slate-700">
                                        {log.type === 'in' ? <span className="text-emerald-500 mr-1">+</span> : <span className="text-emerald-500 mr-1">-</span>}{log.qty} <span className="text-xs font-bold text-slate-400 font-sans tracking-wide">{log.unit}</span>
                                    </td>
